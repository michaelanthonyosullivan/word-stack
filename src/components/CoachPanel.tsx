...
import { HelpCircle, Sparkles, Award, ArrowRight, Eye, GripHorizontal, User } from 'lucide-react';
...
export function CoachPanel({
  ...
  return (
	<div className="w-full flex flex-col gap-3">
	  {/* Turn indicator — desktop only, hidden on mobile via lg:block */}
	  <div className="hidden lg:flex items-center gap-2 px-4 py-2.5 glass-card rounded-2xl border border-white/5">
		<div className={`h-7 w-7 rounded-lg flex items-center justify-center shrink-0 ${
		  isAiTurn ? 'bg-slate-800 text-slate-400' : 'bg-teal-600/15 text-teal-400'
		}`}>
		  <User className="h-3.5 w-3.5" />
		</div>
		<span className={`text-xs font-bold ${
		  isAiTurn ? 'text-slate-400' : 'text-teal-400'
		}`}>
	  ...
	  {/* Post-turn coach analysis — full modal */}
	  ...
	  {/* Compact floating panel while previewing the best move */}
	  {coachAnalysis && showingPreview && (
		<div
