...
echo "▶ Applying MobileHintBar changes..."

# ── 1. Create src/components/MobileHintBar.tsx ────────────────────────────
cat > src/components/MobileHintBar.tsx << 'EOF'
...
import { MobileHintBar } from './components/MobileHintBar';
...
const BUILD_TAG = 'sync-v11-mobile-hint';
	  ...
	  {!gameStarted && !onlineInfo && preGameScreen === 'mode-select' ? (
		...
		<main className="flex-1 flex flex-col lg:flex-row p-4 md:p-6 gap-6 max-w-7xl w-full mx-auto min-h-0">
		  ...
		  <div className="flex-1 flex flex-col items-center gap-4 min-h-0 justify-center">
			...
			{/* Mobile-only hint bar — sits between board and rack, hidden on desktop */}
			<MobileHintBar {...hintProps} />
		  ...
		  <div className="w-full lg:w-[340px] flex flex-col gap-4 shrink-0 overflow-y-auto no-scrollbar max-h-[90vh]">
			...
			<CoachPanel
			  onGetHint={hintProps.onGetHint} onClearHint={clearHint} onAcceptHint={handleAcceptHint}
...
echo "   src/components/MobileHintBar.tsx  (new)"
echo "   src/App.tsx                        (updated)"
echo ""
echo "▶ Run your dev server to test:  npm run dev"
